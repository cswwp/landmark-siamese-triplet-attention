...
def mac(x):
    #print('mac input:', x.shape)
    #print('after mac:', F.max_pool2d(x, (x.size(-2), x.size(-1))).shape)
...
def gem(x, p=3, eps=1e-6):
    #print('mac input:', x.shape)
    #print('after mac:', F.avg_pool2d(x.clamp(min=eps).pow(p), (x.size(-2), x.size(-1))).pow(1./p).shape)
